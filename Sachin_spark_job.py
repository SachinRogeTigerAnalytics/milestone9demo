...
class Data_Transformation():
    ...
    def write_data(self, df, path, partition_cols = []):
        if partition_cols:
            df.repartition(partition_cols[1]).write.mode("overwrite").partitionBy(partition_cols).parquet(path)
    ...
    def scd2_implementation(self, df, lookup_location, pii_cols, dataset):        
        ...
        if dataset == "Viewership":
            df_source = df_source.withColumn(pii_cols[1],lit('null'))
            df_source = df_source.withColumn('masked_'+ pii_cols[1],lit('null'))
